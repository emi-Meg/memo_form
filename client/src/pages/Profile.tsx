  ...
  useEffect(() => {
    const fetchBranchData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8000/api/view-branch`, { headers });
  ...
  useEffect(() => {
    const fetchUserInformation = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8000/api/view-user/${id}`, { headers });
  ...
  const profilePictureUrl = newProfilePic
    ...
    const onSubmit = async () => {
      try {
        ...
        const response = await axios.post(
          `http://localhost:8000/api/update-profilepic/${id}`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${token}`,
