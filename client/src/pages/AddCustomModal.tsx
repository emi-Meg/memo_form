...
const AddCustomModal: React.FC<AddCustomModalProps> = ({
  ...
  console.log('data',notedBy, approvedBy);
  useEffect(() => {
    if (modalIsOpen) {
...
const toggleNotedBy = (approver: Approver) => {
  ...
  setNotedBy((prevNotedBy) => {
    if (isNoted) {
      return prevNotedBy.filter((a) => a.id !== approver.id);
    } else {
      return [...prevNotedBy, approver];
    }
  });
};
  ...
  const toggleApprovedBy = (approver: Approver) => {
    ...
    setApprovedBy((prevApprovedBy) => {
      if (isApproved) {
        return prevApprovedBy.filter((a) => a.id !== approver.id);
      } else {
        return [...prevApprovedBy, approver];
      }
    });
  };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 overflow-y-auto h-80">
            <div>
             ...
             {filteredApprovers.map((approver, index) => {
               ...
               const isChecked = notedBy.some((a) => a.id === approver.id);
               ...
               return (
                 <div key={index} className="flex items-center mb-2">
                   <input
                     ...
                     id={`noted_by_${index}`}
                     checked={isChecked || isNoted}
                     onChange={() => {
                      if (!isApproved) {
                        if (isNoted || isChecked) {
            ...
            <div>
              ...
              {filteredApprovers.map((approver, index) => {
                ...
                const highlightClass =
                  ...
                  const isChecked = approvedBy.some((a) => a.id === approver.id);
